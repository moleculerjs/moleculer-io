...
module.exports = {
  ...
  methods: {
    ...
    makeIOHandler: function makeIOHandler(handlerItem) {
      ...
      return (() => {
        var _ref = _asyncToGenerator(function* (socket, action, params, respond) {
          ...
          try {
            let res = yield svc.actions.call({ socket: socket, action, params, opts, handlerItem });
    ...
    call: {
      ...
      handler(ctx) {
        ...
        return _asyncToGenerator(function* () {
          ...
          if (handlerItem.whitelist && !_this2.checkIOWhitelist(action, handlerItem.whitelist)) {
            debug(`Service "${action}" not found`);
            throw new ServiceNotFoundError({ action });
          }
          const endpoint = _this2.broker.findNextActionEndpoint(action);
          ...
          let meta = _this2.getMeta(socket);
          ...
          if (handlerItem.before) {
            yield handlerItem.before.call(_this2, ctx, socket, args);
          }
          let res = yield ctx.call(args.action, args.params, args.callOptions);
          if (handlerItem.after) {
            yield handlerItem.after.call(_this2, ctx, socket, res);
          }
          _this2.saveUser(socket, ctx);
          if (ctx.meta.$join) {
            yield _this2.joinRooms(socket, ctx.meta.$join);
          }
          if (ctx.meta.$leave) {
            if (_.isArray(ctx.meta.$leave)) {
              yield Promise.all(ctx.meta.$leave.map(function (room) {
                return _this2.leaveRoom(socket, room);
              }));
            } else {
              yield _this2.leaveRoom(socket, ctx.meta.$leave);
            }
          }
          return res;
        })();
