      ...
      for (let eventName in events) {
        switch (events[eventName].type || 'call') {
          case 'call':
            ...
            break;
          case 'login':
            this.handlers[nsp][eventName] = this.makeLoginHandler(eventName, events[eventName].whitelist, events[eventName].callOptions);
            break;
          default:
    ...
    makeLoginHandler: function makeLoginHandler(eventName, whitelist, opts) {
      ...
      return (() => {
        var _ref2 = _asyncToGenerator(function* (action, params, respond) {
          ...
          handler.call(this, action, params, function (err, res) {
            if (err) return respond(err);
            _this2.client.user = res;
            respond(err, res);
          });
        });
